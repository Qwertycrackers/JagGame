...
class Unit {
	...
	Unit(SDL_Renderer*); //!<\brief Set the renderer. Prefer the use of this constructor.
	...
	void receiveEvents(std::vector<std::shared_ptr<SDL_Event>>*); //!<\brief Give this `Unit` a stack of events to handle. Once per frame.
	virtual ~Unit(); 
	protected:
	...
	std::vector<std::shared_ptr<SDL_Event>> *unitEvents; //!<\brief a C-style array of `vector`s containing the events to be passed to each sub-`Unit`. 
